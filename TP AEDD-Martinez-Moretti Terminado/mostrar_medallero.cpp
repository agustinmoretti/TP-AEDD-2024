...
void medalleroPaises(int deporte_medallas[][3], int medallasPorPaises[][3])
{
	int medallasTotales[197], indicesPaises[197], oroPaises[197], plataPaises[197], broncePaises[197];
	...
	cout << setw(9) << "Posicion " << setw(30) << left << " Pais"
		<< setw(8) << left << " Oro"
		<< setw(8) << "Plata"
		<< setw(8) << "Bronce"
		<< setw(8) << "Total" << endl;
	...
	for (int i = 1; i < 197; i++)
	{
		if (medallasTotales[i] != 0)
		{
			cout << setw(9) << right << i << ". ";
			Sleep(50);
			cout << setw(30) << left;               // Alinear el nombre del país a la izquierda
			imprimirPais(indicesPaises[i]);         // Imprime el nombre del país
			cout << setw(8) << left << oroPaises[i] // Valores alineados a la derecha
...
void medalleroDeportes(int deporte_medallas[][3], int medallasPorPaises[][3])
{
	barradecarga(1000, 60);
	Sleep(400);
	system("cls");
...
void medalleroTop(int medallasPorPaises[][3])
{
	int top = 0, medallasTotales[197], indicesPaises[197]; // numero de medallas totales por pais
	int pasada = 1, k, intercambios = 1;
	...
	for (int j = 0; j < 3; j++)
	{
		for (int i = 1; i < 197; i++)
		{
			if (medallasPorPaises[i][j] != 0)
			{
				medallasTotales[i] += medallasPorPaises[i][j];
			}
		}
	}
	...
	for (int i = 1; i <= top; i++)
	{
		...
		cout << setw(30) << left;       // Alinear el nombre del país a la izquierda
		imprimirPais(indicesPaises[i]); // Imprime el nombre del país
		cout << setw(8) << medallasTotales[i] << endl;
		Sleep(50);
	}
...
void mostrarmedallas(int deporte_medallas[][3])
{
	...
	do
	{
		...
		case '1':
			...
			Sleep(1000);
			medalleroPaises(deporte_medallas, medallasPorPaises);
			break;
		case '2':
			// Lógica para Medallero por Deporte
			cout << "Mostrando Medallero por Deporte..." << endl;
			Sleep(1000);
			medalleroDeportes(deporte_medallas, medallasPorPaises);
			break;
		case '3':
			// Lógica para Top Medallero
			cout << "Mostrando Top Medallero..." << endl;
			Sleep(1000);
			medalleroTop(medallasPorPaises);
			break;
		case 'X':
		case 'x':
